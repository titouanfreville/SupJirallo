...
then
  ...
  case $response in
       [nN][oO]|[nN])
         ...
         ;;
      *)
        echo -e "$red The file is going to be overwrite with the defaul config. A copy is going to be made in env_file.old.$basic"
        cp -f env_file env_file.old
        source env_file
        echo "Saving Done"
        ;;
  esac
...
if [ $ENV -eq 0 ]
then
  RES=$(init_checklist "Quick_conf for softgallery Docker" "Select parameters to update -- space to select, enter to validate." env_file.old 23)
  echo $RES
  scripts/create_env_file.sh "$RES"
